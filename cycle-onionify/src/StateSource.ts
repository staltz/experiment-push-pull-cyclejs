...
export class StateSource<T> {
  public stateS: Signal<T | undefined>;
  private _name: string;

  constructor(signal: Signal<T | undefined>, name: string) {
    this.stateS = signal;
    // .filter(s => typeof s !== 'undefined');
    // .compose(dropRepeats())
    // .remember();
    this._name = name;
    (this.stateS as Signal<T> & DevToolEnabledSource)._isCycleSource = name;
  }
  ...
  public select<R>(scope: Scope<T, R>): StateSource<R> {
    const get = makeGetter(scope);
    return new StateSource<R>(this.stateS.map(get), this._name);
  }
